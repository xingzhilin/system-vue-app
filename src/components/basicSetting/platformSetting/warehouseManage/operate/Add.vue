							...
							<td>
								<ul class="hc_wrap">
									<li v-for="item,index in hcInserts">
										...
										<el-button type="text" size="small" class="btn_insert" @click="dialogInsertVisible = true;handleInsertDW(index)">{{index}}录入堆位</el-button>
										<el-dialog title="录入堆位" :visible.sync="dialogInsertVisible">
											<h5 class="dw_insert">{{hcNameAll}}---{{index}}</h5>
											<h6 class="dw_insert">堆位名称</h6>
											<el-form :model="form">
												<p v-for="item in dwInserts"><el-input v-model="item.dwName" auto-complete="off"></el-input></p>
												<el-button size="small" @click="handleDWAdd(item, index)">+ 添加</el-button>
											</el-form>
...
<script>
	...
	export default{
		...
		methods:{
			...
			handleInsertDW(index){
				console.log('index: ' , index);
				this.hcNameAll = this.hcInserts[index].hcName;
				this.dwInserts = [];
			},
			handleDWAdd(item, index){
				console.log('************************************');
				console.log(item);
				console.log(index);
			...
			handleDWdata(){				
				...
				this.dwInsertsTatal.push(this.dwInserts);
				console.log('****************dwInsertsTatal********************');
				console.log(this.dwInsertsTatal);
			},
			handleAddMoney(){
