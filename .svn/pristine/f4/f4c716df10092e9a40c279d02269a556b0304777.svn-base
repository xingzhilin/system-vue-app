<template>
    <div class="category">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="150px" class="demo-ruleForm" :label-position="$store.getters.isPc==true?'right':'top'">
        <el-form-item label="品类名称：" prop="catName">
          <el-col :xs="{span:24}" :sm="{span:24}" :md="{span:10}" :lg="{span:10}" :xl="{span:8}">
            <el-input v-model="ruleForm.name" size="small" :disabled="isEditor"></el-input>
          </el-col>
        </el-form-item>
        <el-form-item label="归属频道：" prop="industryCode">
          <el-col :xs="{span:24}" :sm="{span:24}" :md="{span:10}" :lg="{span:10}" :xl="{span:8}">
            <el-select v-model="ruleForm.industryCode" placeholder="请选择" size="small" :disabled="isEditor">
              <el-option v-for="item in industryCodeOptions" :label="item.industryName" :value="item.industryCode" :key="item.industryCode"></el-option>
            </el-select>
          </el-col>
        </el-form-item>
        <el-form-item label="品类参数：" prop="catTeParams">
            <el-tag v-for="(item,index) in ruleForm.catTeParams" :key="index" size="medium" closable @close="handleClose(item)">{{item}}</el-tag>
        </el-form-item>
        <el-form-item label="添加关键词：">
          <el-col class="message" :xs="{span:24}" :sm="{span:24}" :md="{span:10}" :lg="{span:10}" :xl="{span:8}">
            <el-input v-model="message" size="small"></el-input>
          </el-col>
          <el-button @click="handleAdd(message)" size="small" type="primary">确定</el-button>
        </el-form-item>
        <el-form-item label="备注：" prop="remarks">
          <el-col :xs="{span:24}" :sm="{span:24}" :md="{span:10}" :lg="{span:10}" :xl="{span:8}">
            <el-input type="textarea" :rows="4" resize="none" v-model="ruleForm.remarks"></el-input>
          </el-col>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="submitForm('ruleForm')">保存</el-button>
          <el-button @click="$router.go(-1)">取消</el-button>
        </el-form-item>
      </el-form>
    </div>
</template>
<script>
  export default {
    data() {
      return {
        isEditor:false,
        message:'',
        ruleForm: {
          catName: '',
          industryCode: '',
          catTeParams: [],
          remarks: ''
        },
        industryCodeOptions:[
          {
            value:'1',
            label:'煤炭'
          },
           {
            value:'2',
            label:'钢铁'
          }
        ],
        rules: {
          industryCode: [
            { required: true, message: '请选择归属频道', trigger: 'change' }
          ],
          catTeParams:[
            { required: true, message: '请选添加品类参数', trigger: 'change' }
          ],
          remarks: [
            { min: 0,max:200, message: '长度在 0 到 200 个字符', trigger: 'blur' }
          ]
        }
      };
    },
    created(){
      this.getIndustryCodeList();
    },
    methods: {
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            console.log(this.ruleForm);
          } else {
            console.log('error submit!!');
            return false;
          }
        });
      },
      handleClose(tag) {
        this.ruleForm.catTeParams.splice(this.ruleForm.catTeParams.indexOf(tag), 1);
      },
      handleAdd(message){
        if(message == ''){
            return;
        }
        this.ruleForm.catTeParams.push(message);
        this.$refs.ruleForm.validateField('catTeParams');
      },
      getIndustryCodeList(){
        // this.$http.get('/api/commons/industries').then(res=>{
               // this.industryCodeOptions = res.list;
                
            // });
            var res = {
          "status": 200,
          "message": "操作成功",
          "list": [
            {
              "industryCode": "14",
              "industryName": "金银岛一号"
            },
             {
             "industryCode": "12",
              "industryName": "金银岛一号"
            }]
        };
        this.industryCodeOptions = res.list;
       }
    }
  }
</script>
<style scoped lang="scss">
    .category {
      padding: 30px;
      .el-tag {
        margin-right: 10px;
      }
      .message {
        margin-right:10px;
      }
    }
</style>